...
const SuggestionsItem: React.FC<SuggestionsItemProps> = ({ feedback, isLink = false }) => {
  ...
  return (
    <div className="flex w-full flex-wrap justify-between gap-y-4 rounded-lg bg-white p-6 md:flex-nowrap md:px-8 md:py-7">
      {isLink ? (
        <Link href={`/feedbacks/${feedback.id}`} className="w-full gap-y-2 md:ml-10 md:mr-6">
          {details}
        </Link>
      ) : (
        <div className="w-full gap-y-2 md:ml-10 md:mr-6">{details}</div>
      )}

      <button
        aria-label={`Upvote feedback, current upvotes: ${upvotes}`}
        className="w-fit rounded-lg bg-light-300 px-4 py-1.5 font-semibold text-blue-300 hover:bg-light-400 md:order-first md:h-fit md:max-w-10 md:pb-2 md:pt-3"
      >
